...
const registerController = async (req,res) => {
    try {
        ...
        await user.save()
        return res.status(201).send({ // 201 means something has been created
            success:true,
...
const loginController = async (req,res) => {
    try {
        ...
        if(existingUser.role !== req.body.role) {
         ...
         // this sign is used to create token
         const token = jwt.sign({userId:existingUser._id},process.env.JWT_SECRET,{expiresIn: '1d'});
         // when this 'token' get expired , user have to login again in this case expires is in 1 day 
